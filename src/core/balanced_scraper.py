...
from ..classifiers.balanced_classifier import BalancedClassifier, StrictnessLevel
...
class BalancedScraper:
    ...
    def __init__(self, config: ScraperConfig, strictness: str = "balanced"):
        ...
        self.classifier = BalancedClassifier(strictness=strictness_level)
    ...
    def scrape_all_products(self) -> Dict[str, Dict]:
        ...
        """
        print("\n" + "="*80)
        print("PRODUCT CATALOG SCRAPER - BALANCED MODE")
        print("="*80)
        print(f"Strictness: {self.classifier.strictness.value.upper()}")
        print(f"Threshold: {self.classifier.PRODUCT_THRESHOLD}")
        print("="*80)
