...
"""
    ...
    def extract_with_gemini_playwright(self, url: str, configurator_info: Dict) -> Dict:
        ...
        try:
            # Use Gemini interactive extractor
            options = self.gemini_extractor.interactive_extraction(
                url,
                max_iterations=10
            )
    ...
    def scrape_all_products(self) -> Dict:
        ...
        for i, url in enumerate(product_urls, 1):
            ...
            # Scrape the product with LAM method
            product_data = self._scrape_product_with_config_info(url, config_info)
    ...
    def _scrape_product_with_config_info(self, url: str, config_info: Dict) -> Optional[Dict]:
        ...
        if config_info.get('requires_interaction') and self.gemini_extractor:
            ...
            result = self.extract_with_gemini_playwright(url, config_info)
            if result['success']:
